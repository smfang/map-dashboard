use sp1_sdk::{ProverClient, SP1Proof, SP1PublicValues, SP1Stdin, SP1Stdout};

// SP1 program for carbon index calculation proof generation
// This program proves the correctness of carbon index calculations based on NetCDF data
...
fn main() {
    ...
    println!("NHI Score: {}", result.nhi_score);
    println!("Carbon Index Value: {}", result.carbon_index_value);
}
...
struct CarbonIndexResult {
    ...
    gamma_scale: f64,
    gamma_loc: f64,
    carbon_index_value: u64, // Scaled by 1e18 for fixed-point arithmetic
}
...
mod tests {
    ...
    #[test]
    fn test_carbon_index_calculation() {
        let rainfall_data = vec![
            25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0,
            75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0, 120.0
        ];
        let current_rainfall = 45.0;
        
        let result = calculate_carbon_index(&rainfall_data, current_rainfall);
        
        assert!(result.nhi_score >= 0.0 && result.nhi_score <= 1.0);
        assert!(result.carbon_index_value > 0);
        assert!(result.p20_percentile > 0.0);
        assert!(result.p90_percentile > 0.0);
    }
}
